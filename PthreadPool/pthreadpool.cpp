#include <stdio.h>
#include <unistd.h>
#include <pthread.h>
#include <queue>


using namespace std;
typedef void (*Handler)(int);

class QueueData
{
  public:
    QueueData(int data,Handler hander)
    {
      data_ = data;
      hander_ = hander_;
    }
    void run()
    {
      hander_(data_);
    }
  private:
    int data_;
    Handler hander_;

};

class ThreadPool
{
  public:
    ThreadPool(int capacity,int pthread_count)
    {
      capacity_ = capacity;
      pthread_count_ = pthread_count;

      pthread_mutex_init(&lock_,NULL);
      pthread_cond_init(&cond_,NULL);

      flag = 0;
    }
    ~ThreadPool()
    {
      pthread_mutex_destroy(&lock_);
      pthread_cond_destroy(&cond_);
    }
    int Oninit()
    {
      pthread_t tid;
      for(int i = 0;i < pthread_count_;i++)
      {
        int ret = pthread_create(&tid, NULL, Poolthread, (void*)this);
        if(ret < 0)
        {
          perror("pthread_create");
          return -1;
        }
      }
      return 0;
    }
    int Push(QueueData* qd)
    {
      pthread_mutex_lock(&lock_);
      if(flag)
      {
        pthread_mutex_unlock(&lock_);
        return -1;
      }
      que_.push(qd);
      pthread_cond_broadcast(&cond_);
      return 0;
    }
    void threadExit()
    {
      pthread_mutex_lock(&lock_);
      flag = 1;
      pthread_mutex_unlock(&lock_);
      pthread_cond_broadcast(&cond_);
    }
  private:
    void Pop(QueueData** qd)
    {
      *qd = que_.front();
      que_.pop();
    }

    static void* Poolthread(void* arg)
    {
      pthread_detach(pthread_self());
      ThreadPool* tp = (ThreadPool*)arg;
      while(1)
      {
        pthread_mutex_lock(&tp->lock_);
        while(tp->que_.empty())
        {
          if(tp->flag)
          {
            tp->pthread_count_--;
            pthread_mutex_unlock(&tp->lock_);
            pthread_exit(NULL);
          }
          pthread_cond_wait(&tp->cond_,&tp->lock_);
        }
        QueueData* qd;
        tp->Pop(&qd);
        pthread_mutex_unlock(&tp->lock_);

        qd->run();
        delete qd;
      }
    }
  private:
    queue<QueueData*> que_;
    int capacity_;

    pthread_mutex_t lock_;
    pthread_cond_t cond_;

    int pthread_count_;
    int flag;
};


void dealData(int data)
{
  printf("data:%d\n",data);
}
int main()
{
  ThreadPool* tp = new ThreadPool(4,2);
  if(!tp)
  {
    printf("Create ThreadPool failed!\n");
    return -1;
  }
  int ret = tp->Oninit();
  if(ret < 0)
  {
    printf("Create ThreadPool falied!\n");
    return -1;
  }
  for(int i = 0;i < 100;i++)
  {
    QueueData* qd = new QueueData(i,dealData);
    if(!qd)
    {
      continue;
    }
    tp->Push(qd);
  }
  sleep(6);
  tp->threadExit();
  delete tp;

  return 0;
}
